...
def menu():  # A "menu()" function is defined that will serve the purpose of choosing what action the employee will take when the credentials entered at the log-in page are correct.
    ...
    while True:
        ...
        programSelect = input(
            "Enter number 1 through 11: ")  # A variable "programSelect" is defined which stands for the input for the user to choose the action that needs to be taken.
        # Lastly a set of conditionals is made and the appropriate function is called for further action.
        if programSelect == "1":
            placeOrder()
            ...
            # If the number eleven (11) is entered the loop breaks and it returns to the log-in page.
            break
        else:
            # If the number is incorrect this message pops up.
            print(
                "Wrong input! Please enter a number and make sure it is an integer 1 through 11.")
...
def numOrdersSpecificCustomer():
    ...
    # Generator expression to see the number of times where the name in the customer's list matches the input entered by the user.
    occurrences = sum(
...
def totalOrdersSpecificDate():
    ...
    # Generator expression to see the number of times where the date in the customer's list matches the input entered by the user.
...
def totalAmountSpecificCustomer():
    ...
    for customer in customersList:
        print(customer[0])
    inputName = input(
        "\nThe following customers have placed orders. Enter the name you want to look up to see the total amount of a specific customer: ")  # Input is made.
...
def totalAmountSpecificDay():
    for date in customersList:  # Iterates the dates in the "customersList".
        print(date[2])
    inputDate = input(
        "\nThe following dates there were orders placed. Write a date to see the total amount of orders placed that day: ")  # Input is entered from the user.
